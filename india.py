...
def get_grade_point_india():
    # File Path
    file_path = "Updated University List India1.xlsx"
    ...
    # Load DataFrames
    university_df = pd.read_excel(xls, sheet_name="India University List")
    format_df = pd.read_excel(xls, sheet_name="India Formats", header=None)
    ...
    # university_mapping = {
    #     row["University List"].strip().lower(): [fmt.strip().lower() for fmt in row["Format"]]
    #     for _, row in university_df.iterrows()
    # }

    # print("Inside India grade point")
    # print("Available universities:", university_mapping.keys())
    # print("Available formats:", format_df["Format"].unique())
    
    import sys
    # university_mapping = {}
    # for _, row in university_df.iterrows():
    #     university = row["University List"].strip().lower()
    #     formats = [fmt.strip().lower() for fmt in row["Format"].split(" / ")]
    #     print(f"University: {university} -> Formats: {formats}")  # 👈 Debug print
    #     university_mapping[university] = formats   

    print([fmt.strip().lower() for fmt in row["Format"].split(" / ")])
    # Create Mapping
    university_mapping = {
